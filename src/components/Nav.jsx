    ...
    <header className="flex justify-center">
      ...
      <div className="flex items-center ml-6">
        ...
        <img
          ...
          className="w-[calc(100%-34%)] md:w-full"
        />
        </Link>
      </div>
      ...
      >
        <img src={toggle ? menu : close} className="animate-bounce w-6" />
        <div
        className={`fixed top-[4rem] p-2 rounded-xl bg-black/80 backdrop-blur-xs z-50 ${
          toggle ? "hidden" : "flex"
        }`}
        >
        <ul
          className={`list-none relative ${toggle ? "hidden" : "flex"} flex flex-col justify-center item-end`}
        >
          {Menu.map((item, id) => (
